...
const NewChannelDialog: React.FC<NewChannelDialog> = (props) => {
    ...
    const handleFormSubmit: SubmitHandler<FormData> = (data) => {
		...
		if (Object.keys(errors).length === 0) {
			...
			props.onSubmit(data)
			reset({roomName: "", gameType: LangTranslationKey.OLHO})
			props.setOpen(false)
        }
    };
			...
			<DialogContent className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg sm:max-w-[470px]">
				<form onSubmit={handleSubmit(handleFormSubmit)}>
					...
					<DialogHeader className="relative">
						<DialogTitle className="text-xl font-bold">{lang(LangTranslationKey.CREATE_GAME_ROOM)}</DialogTitle>
						<DialogDescription className="text-muted-foreground">
							{lang(LangTranslationKey.CREATE_ROOM_DIALOG_DESCRIPTION)}
						</DialogDescription>
					</DialogHeader>
					...
					<div className="grid gap-4 py-4">
						<div className="grid grid-cols-4 gap-4 items-center">
							...
							<Label htmlFor="roomName" className="text-right">{lang(LangTranslationKey.ROOM_NAME)}</Label>
							<div className="col-span-3 grid grid-cols-1 gap-1">
								<Input
									id="roomName" 
									placeholder={lang(LangTranslationKey.ROOM_NAME)}
									className=""
						...
						<div className="grid grid-cols-4 items-center gap-4">
							<Label htmlFor="gameType" className="text-right">{lang(LangTranslationKey.GAME)}</Label>
							<Controller
								name="gameType"
								control={control}
								defaultValue={LangTranslationKey.OLHO}  // Set a meaningful default value for gameType
								render={({ field }) => (
									<GameTypeSwitch 
										value={field.value}
					...
					<DialogFooter>
						<Button type="submit" className="text-confirm-foreground hover:bg-confirm-hover bg-confirm">
							{lang(LangTranslationKey.CREATE_GAME_ROOM)}
						</Button>
						<DialogClose asChild>
                            <button ref={dialogCloseRef} style={{ display: 'none' }}>{lang(LangTranslationKey.CLOSE)}</button>
                        </DialogClose>
					</DialogFooter>
				</form>
			</DialogContent>
